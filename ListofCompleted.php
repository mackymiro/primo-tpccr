    ...
    <section class="content">
      <div class="row">
        ...
        if ($_SESSION['UserType']=='Admin'){
          ...
          if ($_POST['Search']==''){
            $strSQL="SELECT  * FROM primo_view_jobs Where ProcessCode='ContentStructure' AND statusstring='DONE' and status<>'Transmitted'";  
          }
          else{
            $strSQL="SELECT * FROM primo_view_jobs Where ProcessCode='ContentStructure' AND statusstring='DONE' and  status<>'Transmitted' AND Jurisdiction='".$_POST['Search']."'";   
          }
         
        }
        else{
          if ($_POST['Search']==''){
            $strSQL="SELECT * FROM primo_view_jobs Where ProcessCode='ContentStructure' AND statusstring='DONE' and status<>'Transmitted' AND AssignedTo='$_SESSION[login_user]'";  
          }
          else{

             $strSQL="SELECT  * FROM primo_view_jobs Where ProcessCode='ContentStructure' AND statusstring='DONE' and status<>'Transmitted' AND AssignedTo='$_SESSION[login_user]' AND Jurisdiction='".$_POST['Search']."'";  
          }
        ...
        if($encoding == "UTF-8")
          ...
          $out = iconv(mb_detect_encoding($Title, mb_detect_order(), false), "UTF-8//IGNORE",$Title);
				...
				?>
                  ...
                  <td><!--<a href="index.php?file=<?= $filename;?>&BatchID=<?= $row["BatchId"];?>&Status=Completed">--><?= $row["JobName"];?></td>
